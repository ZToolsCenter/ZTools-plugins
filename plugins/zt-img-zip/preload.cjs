const IMAGE_ZIP_PRELOAD_VERSION = "0.1.4";
...
function resolveBundledModule(moduleName) {
  return require(path.join(__dirname, "preload", "node_modules", moduleName));
}

function getSharp() {
  if (!sharpInstance) {
    try {
      sharpInstance = resolveBundledModule("sharp");
    } catch (bundledError) {
      try {
        sharpInstance = require("sharp");
      } catch (fallbackError) {
        const error = new Error(
          `无法加载 sharp 运行时依赖，请确认插件包包含 preload/node_modules/sharp: ${fallbackError.message}`,
        );
...
function resizePipeline(pipeline, options) {
...
async function compressImage(filePath, options = {}) {
  ...
  pipeline = resizePipeline(pipeline, options);
