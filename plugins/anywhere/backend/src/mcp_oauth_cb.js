 ...
 */
async function runAuthFlowWithFallback({ authorizeUrl, expectedState, preferredPort = 0, timeoutMs, loopback = null }) {
  ...
  try {
    ...
    const params = await managedLoopback.fetchCallbackParams;
    return params;
  } catch (loopbackErr) {
    if (managedLoopback && managedLoopback.cleanup) managedLoopback.cleanup();
