...
/**
 * `⌘1`–`⌘9`：粘贴**本屏**第 N 行（09-23 改的口径，以前是"列表第 N 条"）。
 *
 * ⚠️ **跟 ⌘K 同一个前提：焦点得先在插件里**（先按一次 ↑↓、或者用鼠标点过列表）。
 * 数字键**不在**宿主那六个键的白名单里，所以「打开插件就直接按 ⌘1」是收不到的 ——
 * 这不是能靠改插件绕开的事（详见 REFERENCE §26.1-D）。
 *
 * ⚠️ 起点是**屏首**（屏幕最上面那一行），不是列表开头 —— 所以翻页 / 下滑之后
 * 每一屏都有键可用。换算在 `lib/viewport.ts`（纯函数 + 单测），量屏首在 App.vue。
 *
 * ⚠️ 序号**必须**跟这一套同源（App.vue 的 `rowNums` 走同一个 `screenNumbers`），
 * 不能另算一份 —— 两处一旦不一致，按 ⌘3 粘到的就不是屏幕写着 3 的那一行。
 */
...
function withMod(key: string): KeyAction | null {
  ...
  switch (key) {
     ...
     *
     * 来由：行尾那颗 ✎ 加了开关（设置里能关）；关掉之后编辑就只剩这一条路 ——
     * 跟 `tailFav` 关掉还剩 ⌘K、`tailDel` 关掉还剩 Delete 是同一个规矩：
     * **按钮管鼠标、键位管键盘，两边各有一条路，关哪边都不把功能关死。**
     *
     * ⚠️ 只在**收藏视图的文本行**上真的生效（判定在 App.vue 的 `editActive()`）——
     *    历史那一摊是宿主的账，插件改不了；图片/文件没有"正文"可改。
     *    在别的视图按它**什么都不会发生**，这是对的，不是漏了。
     *
     * ⚠️ 宿主没占 ⌘E：插件视图的 `before-input-event` 只拦 ⌘D（分离）和 ⌘Q（退出），
     *    另外主窗口拦了 ⌘R / ⌘W / 缩放那几个 —— 都不含 E（已读 `app.asar` 核对，见 REFERENCE §49）。
     *    跟 ⌘K / ⌘C / ⌘L 同一个前提：**焦点得先在插件里**（搜索框握着焦点时 ⌘ 组合键在渲染层就被丢了）。
     */
    case 'e':
      return 'editFavorite'
    /*
     ...
     *
     * 来由：右下角那颗「新增」也加了开关（设置里能关）。关掉之后**鼠标就没有入口了**，
     * 而底栏这几颗按钮的规矩是「按钮管鼠标、键位管键盘，关哪边都不把功能关死」
     * （★ 收藏 / ⌘E 编辑 / Delete 删除全是这么配的）—— 新增原先**没有**对应的键，
     * 是这一串里唯一的缺口（原因也简单：它是插进来的第二样东西，不是从按钮挪过来的）。
     * 09-24 老大看到这个缺口，说「⌘N 是空的的话，可以配一个」，于是补上。
     *
     * ⚠️ 跟"那颗按钮显不显示"**无关**（09-24 之前是 `footAdd` 那个开关，
     *    现在是 `footButtons` 多选里的「新增」）：设置只管"界面上画不画那颗按钮"，
     *    这条键不看它 —— 它正是**关掉按钮之后的那条路**（跟 ⌘E 不看 `tailEdit` 完全同构）。
     *
     * ⚠️ 只在**收藏视图**里真的生效（判定在 App.vue）：新增出来的东西本来就落在收藏里，
     *    在历史那边弹出这个框会让人以为在改历史。在别的视图按它**什么都不会发生**，这是对的。
     *
     * ⚠️ 宿主没占 ⌘N：插件视图的 `before-input-event` 只拦 ⌘D / ⌘Q，主窗口拦的是
     *    ⌘R / ⌘W 和 ⌘= ⌘- ⌘0 那几个缩放 —— **都不含 N**（已读 `app.asar` 核对，见 REFERENCE §49）。
