...
- **图片文件带缩略图**：单个 `.png` / `.jpg` 之类的图片文件，行首直接显示缩略图
...
| 单击行              | 选中                                                                   |
| 双击行              | 复制（窗口不关）                                                       |
| 行尾`☆` / `🗑` | 收藏 / 删除当前项（鼠标划上去才出现，两枚可在设置里各关各的）          |
...
>
> 翻页那两族里，`PageDown` / `PageUp` 要先按一次 `↑↓`（或者用鼠标点一下列表）才收得到；`⌘↓` / `⌘↑`（Windows 上是 `Ctrl+↓` / `Ctrl+↑`）不用 —— 它俩是搜索框直接转发过来的，打开插件就能按。
...
| `Enter`     | 落值 / 切换（「行尾操作」「行尾显示」那五颗、以及两个开关，都靠它）  |
| `Esc`       | 关面板，回到列表                                                     |

有几点跟直觉不太一样，是有意的：

- **「行尾操作」「行尾显示」那五颗都是各自独立的事**（收藏 / 删除两颗按钮；类型 / 序号 / 来源三样标签，都可开可关，也可以全关），所以 `←→` 只挪光标、**按 `Enter` 才真的切换**。要是做成"移到哪颗就点亮哪颗"，从「收藏」滑到「删除」就会顺手把删除也点亮。
- **收藏按钮关掉之后，`⌘K` 收藏完会闪一颗星** —— 鼠标那个入口没了，就补一句"刚才成了"的回执（约 1 秒后淡掉）。按钮开着的时候不闪，因为按钮自己会点亮 / 熄灭。
...
- **复制一个图片文件，它归「文件」不归「图像」**：宿主按「剪贴板上放的是文件还是图片」分类，跟后缀名无关 —— 从访达 / 资源管理器复制 `.png` 是**文件**（能按文件名搜到），截图或在网页里「复制图像」才是**图像**。两种都会显示缩略图
...
- **收藏**：这一颗关掉之后，鼠标就不能收藏了，只能按 `⌘K`。这时按 `⌘K` 会在那一行闪一颗星（约 1 秒后淡掉），
  补上"刚才那一下成功了"；开着的时候不闪 —— 按钮自己会点亮 / 熄灭，行上就写着状态
- **删除**：关掉之后只剩 `Delete`（删掉的东西当场消失，本身就是反馈，所以不额外闪）

**两颗都关**也是可以的，那就跟"行尾没有按钮"一样：鼠标只剩「单击选中、双击复制」，
收藏和删除都走键盘。

这两枚按钮跟下面三样标签**叠在同一格**，所以开着的时候，**鼠标划过那一行、或者那一行正好是当前选中行**，
标签 / 序号 / 来源都会让位给按钮（两者表现完全一致，不会同一行换个操作方式就长得不一样）。
