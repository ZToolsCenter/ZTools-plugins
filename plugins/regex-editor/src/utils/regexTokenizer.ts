...
function parseGroupHeader(pattern: string, start: number): {
  ...
  const marker = pattern.slice(start + 2, start + 4)
  if (marker === '?:') return { kind: 'non-capturing', contentStart: start + 4 }
  if (marker === '?=') return { kind: 'lookahead', contentStart: start + 4 }
  if (marker === '?!') return { kind: 'negative-lookahead', contentStart: start + 4 }
  if (marker === '?<') {
    const close = pattern.indexOf('>', start + 3)
    const name = close === -1 ? undefined : pattern.slice(start + 3, close)
    return { kind: 'named', name, contentStart: close === -1 ? start + 3 : close + 1 }
  }
  if (pattern.slice(start + 2, start + 5) === '?<=') {
    return { kind: 'lookbehind', contentStart: start + 5 }
  }
  if (pattern.slice(start + 2, start + 5) === '?<!') {
    return { kind: 'negative-lookbehind', contentStart: start + 5 }
  }
