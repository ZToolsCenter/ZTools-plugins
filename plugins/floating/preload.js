...
function log(str) {
    // fs.appendFileSync('~/Downloads/log.txt', str + "\n")
}
function show(payload, filePath) {
    ...
    img.src = payload;
    img.onload = function () {
        let width = img.width / (ztools.isMacOs() ? 2 : 1)
        let height = img.height / (ztools.isMacOs() ? 2 : 1)
        let scale = width / (height * 1.0)
        //图片大小不能超过当前显示器80%，否则缩放
        let display = ztools.getDisplayNearestPoint(ztools.getCursorScreenPoint())
        if (display) {
        ...
        let imgWin = ztools.createBrowserWindow('suspend.html?#' + imgKey, {
            title: 'img',
            ...
            ipcRenderer.sendTo(imgWin.webContents.id, 'init', imgWin.webContents.id);
            for (var i = 1; i <= 5; i++) {
                setTimeout(() => ipcRenderer.sendTo(imgWin.webContents.id, 'init', imgWin.webContents.id), i * 200);
            }
            ipcRenderer.on('resize', async (event, data) => {
                if (data.winId == imgWin.webContents.id) {
                    let nowBounds = imgWin.getBounds()
                    let widthChanged = nowBounds.width + data.changed
                    console.log('resize', nowBounds, widthChanged)
                    imgWin.setSize(Math.ceil(widthChanged), Math.ceil(widthChanged * data.proportion));
                }
            });
            ipcRenderer.on('moveBounds', (event, data) => {
                if (data.winId == imgWin.webContents.id) {
                    let bound = imgWin.getBounds();
                    let newBounds = {
                        x: parseInt(bound.x + data.x),
                        y: parseInt(bound.y + data.y),
                        width: parseInt(data.width || bound.width),
                        height: parseInt(data.height || bound.height)
                    }
            ...
            //     }
            // });
            ipcRenderer.on('modifyOpacity', (event, data) => {
                if (data.winId == imgWin.webContents.id) {
                    let opacity = imgWin.getOpacity();
                    imgWin.setOpacity(opacity + data.op);
                }
            })
    ...
    "suspend-screenshot": {
        ...
        args: {
            enter: (action) => {
                ...
                ztools.screenCapture(base64Str => {
                    // ztools.copyImage(base64Str);
                    show(base64Str);
                    ztools.outPlugin();
                })
            }
