...
const RUNTIME_CACHE_ROOT = path.join(os.homedir(), "Library", "Application Support", "ZTools", "native-ocr");
...
async function installRuntime(onProgress) {
  ...
  runtimeInstallPromise = (async () => {
    ...
    const tmpTgz = path.join(RUNTIME_CACHE_ROOT, `native-ocr-runtime-${latest.version}-${Date.now()}.tgz`);
...
function ensureRuntimeScript(scriptPath, targetName) {
  ...
  const target = path.join(os.tmpdir(), targetName);
...
const ONNX_RUNTIME_DIR = path.join(RUNTIME_CACHE_ROOT, "onnx-runtime");
const ONNX_PACKAGES = [
...
function onnxModelsPaths() {
  return {
    detectionPath: path.join(ONNX_RUNTIME_DIR, "assets", "ch_PP-OCRv4_det_infer.onnx"),
    recognitionPath: path.join(ONNX_RUNTIME_DIR, "assets", "ch_PP-OCRv4_rec_infer.onnx"),
    dictionaryPath: path.join(ONNX_RUNTIME_DIR, "assets", "ppocr_keys_v1.txt")
  };
}

function checkOnnxRuntime() {
  const meta = readJsonFile(path.join(ONNX_RUNTIME_DIR, ".native-ocr-onnx.json"));
  const installed = Boolean(
    meta && meta.version === ONNX_RUNTIME_VERSION
    && fs.existsSync(path.join(ONNX_RUNTIME_DIR, "node_modules", "@gutenye", "ocr-common", "package.json"))
    && fs.existsSync(path.join(ONNX_RUNTIME_DIR, "onnx_ocr_server.mjs"))
    && ONNX_ASSETS.every((file) => fs.existsSync(path.join(ONNX_RUNTIME_DIR, "assets", file)))
  );
  return {
    installed,
    ready: installed,
...
async function installOnnxRuntime(onProgress) {
  ...
  onnxInstallPromise = (async () => {
    fs.mkdirSync(ONNX_RUNTIME_DIR, { recursive: true });
    ...
    for (let index = 0; index < ONNX_PACKAGES.length; index += 1) {
      ...
      const tmpTgz = path.join(RUNTIME_CACHE_ROOT, `onnx-${pkg.name.replace(/[^\w.-]/g, "_")}-${pkg.version}.tgz`);
      ...
      if (pkg.assetsOnly) {
        for (const asset of ONNX_ASSETS) {
          extractTarBuffer(tarBuffer, ONNX_RUNTIME_DIR, {
        ...
        extractTarBuffer(tarBuffer, path.join(ONNX_RUNTIME_DIR, "node_modules", pkg.name), {
    ...
    fs.copyFileSync(
      ...
      path.join(ONNX_RUNTIME_DIR, "onnx_ocr_backend.mjs")
    );
    fs.copyFileSync(
      path.join(__dirname, "bin", "onnx_ocr_server.mjs"),
      path.join(ONNX_RUNTIME_DIR, "onnx_ocr_server.mjs")
    );
    fs.writeFileSync(path.join(ONNX_RUNTIME_DIR, ".native-ocr-onnx.json"), JSON.stringify({
...
function ensureOnnxServer() {
  ...
  const scriptPath = path.join(ONNX_RUNTIME_DIR, "onnx_ocr_server.mjs");
