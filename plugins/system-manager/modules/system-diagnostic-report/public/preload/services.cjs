...
const { collectSystemVolumeStats } = require('./collectors/node-probes.cjs')
const {
  createSystemInformationProcessRunner
} = require('./collectors/systeminformation-process-client.cjs')

const MAX_REPORT_BYTES = 20 * 1024 * 1024
const collectionFlights = new Map()

function restrictedWindowsEnvironment(environment) {
  const source = environment && typeof environment === 'object' ? environment : {}
  const result = {}
  if (typeof source.SystemDrive === 'string' && /^[A-Za-z]:$/.test(source.SystemDrive)) {
    result.SystemDrive = source.SystemDrive
  }
  if (typeof source.SystemRoot === 'string' && /^[A-Za-z]:\\Windows$/i.test(source.SystemRoot)) {
    result.SystemRoot = source.SystemRoot
  }
  return Object.freeze(result)
}

const diagnosticProcessInfo = Object.freeze({
  platform: process.platform,
  execPath: process.execPath,
  env: restrictedWindowsEnvironment(process.env),
  versions: Object.freeze({
    node: process.versions.node,
    electron: process.versions.electron
  })
})
const runSystemInformation = createSystemInformationProcessRunner({
...
function collect(options = {}) {
  ...
  const flight = collectSystemReport(
    { privacy },
    {
      ...
      nodeOs,
      getSystemVolumeStats: () => collectSystemVolumeStats({ processInfo: diagnosticProcessInfo }),
      getDisplays,
      getZToolsVersion,
      processInfo: diagnosticProcessInfo
    }
