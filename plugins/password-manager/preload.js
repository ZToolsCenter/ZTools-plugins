...
// ---- 数据库存放位置 ----
// 插件可能以 asar 形式安装（插件目录只读），因此 DB 绝不能放在 __dirname 内。
// 统一落到「用户 .ztools 根目录」下的 password-manager/ 文件夹（始终可读写）。
// 解析优先级：
//   1) 从 __dirname 向上查找同时含 plugins/ 与 version.json|Local State 的目录 → 即 .ztools 根；
//      （asar 模式下 __dirname 形如 .../plugins/password-manager.asar，向上两级即可命中，
//       不依赖 asar 内部路径，且不会落到只读的 asar 目录里）
//   2) 退化为 ~/ .ztools（开发态插件不在 .ztools 目录树内时仍可用）
function resolveZtoolsRoot() {
  let dir = __dirname;
  while (true) {
    try {
      const hasPlugins = fs.existsSync(path.join(dir, 'plugins'));
      const hasMarker = fs.existsSync(path.join(dir, 'version.json')) ||
                        fs.existsSync(path.join(dir, 'Local State'));
      if (hasPlugins && hasMarker) return dir;
    } catch (e) { /* ignore */ }
...
const DB_DIR = path.join(resolveZtoolsRoot(), 'password-manager');
const DB_PATH = path.join(DB_DIR, 'passwords.db');

// 确保目录存在（asar 等只读插件目录外，可正常创建）
try { fs.mkdirSync(DB_DIR, { recursive: true }); } catch (e) { /* ignore */ }

// 旧版本把库放在插件目录内（passwords.db 与 preload.js 同级）；首次启动若新位置无数据、
// 旧位置有数据，则复制过来，避免已有密码丢失。
function migrateLegacyDb() {
  try {
    if (fs.existsSync(DB_PATH)) return;
    const legacy = path.join(__dirname, 'passwords.db');
...
const pmApi = {
  ...
  async getStatus() {
    ...
    // 返回数据库实际落盘位置（便于用户确认数据不在只读的插件/asar 目录内）
    async getDbInfo() {
      return {
        dbPath: DB_PATH,
        dbDir: DB_DIR,
        inAsar: DB_PATH.indexOf('.asar') >= 0,
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { pmApi, ensureReady, DB_PATH, DB_DIR };
}
