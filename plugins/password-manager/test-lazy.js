...
// 隔离 DB：把 DB_DIR 改到临时目录，绝不碰用户 .ztools 下的真实库
const fs = require('fs');
...
// 新架构：DB_DIR = path.join(resolveZtoolsRoot(), 'password-manager')
// 替换为临时目录实现隔离。若结构变更导致替换失效，直接报错而非误用真实库。
const dbDirLiteral = "'" + tmpDbDir.replace(/\\/g, '\\\\') + "'";
src = src.replace(
  "path.join(resolveZtoolsRoot(), 'password-manager')",
  "path.join(" + dbDirLiteral + ", 'password-manager')"
);
if (src.indexOf(dbDirLiteral) < 0) {
  throw new Error('DB 路径隔离补丁未生效：preload.js 结构可能已变更');
}
...
(async () => {
  ...
  for (let i = 3; i <= 1001; i++) {
    await pmApi.addHost('10.0.' + Math.floor(i / 254) + '.' + (i % 254), [{ label: 'k', value: 'v' + i }]);
  }
