...
# 密文库（运行时自动生成，独立于插件目录 —— 规避 asar 只读）。同一台机器的所有安装共用
C:\Users\<你>\.ztools\password-manager\
└── passwords.db      # 密文，切勿手动改动/删除
```
...
# 注意：密文库在插件目录【之外】的 C:\Users\<你>\.ztools\password-manager\passwords.db，
#       同步代码时不要动它；唯此一份，换机/重装系统即丢失。
```

> **asar / 市场安装注意**：从市场安装时插件会被打包为 `.asar`（目录只读），`preload.js` 无法在插件目录内创建 `passwords.db`。本插件已把密文库统一放到可读写的 `.ztools\password-manager\` 根目录（首次运行自动建文件夹），因此 asar 下也能正常写库。升级插件时只替换插件目录内的代码文件，`.ztools\password-manager\passwords.db` 不受影响。
...
2. 设置完成后即在 `C:\Users\<你>\.ztools\password-manager\passwords.db` 初始化本地密文库，进入主界面。
...
- **存储**：`sql.js`（纯 WASM SQLite），本地文件 `passwords.db`，位于 `.ztools\password-manager\`（独立可读写目录，兼容 asar 只读安装）。
...
| `getDbInfo()` | 返回密文库落盘位置 `{ dbPath, dbDir, inAsar, exists }`（确认不在只读插件/asar 目录内） |
