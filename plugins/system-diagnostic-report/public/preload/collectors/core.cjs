...
function callSystemInformation(si, methodName) {
  ...
  const flight = Promise.resolve().then(() => si[methodName]())
...
function runWithTimeout(source, task, timeoutMs, clock = Date.now) {
  const startedAt = clock()
  let timeoutId

  const timeoutPromise = new Promise((resolve, reject) => {
    timeoutId = setTimeout(() => reject(new CollectorTimeoutError(source, timeoutMs)), timeoutMs)
  })

  return Promise.race([Promise.resolve().then(task), timeoutPromise])
    .then((value) => ({ value, durationMs: Math.max(0, clock() - startedAt) }))
...
function buildCollectors(dependencies) {
  const { si } = dependencies
  return [
    {
      source: 'os',
      provider: 'systeminformation',
      run: async () => normalizeOs(await callSystemInformation(si, 'osInfo')),
      assign(report, value) {
        report.os = value
    ...
    },
    {
      source: 'device',
      provider: 'systeminformation',
      run: async () => normalizeDevice(await callSystemInformation(si, 'system')),
      assign(report, value) { report.device = value }
    },
    {
      source: 'cpu',
      provider: 'systeminformation',
      run: async () => normalizeCpu(await callSystemInformation(si, 'cpu')),
      assign(report, value) { report.cpu = value }
    },
    {
      source: 'memory',
      provider: 'systeminformation',
      run: async () => normalizeMemory(await callSystemInformation(si, 'mem')),
      assign(report, value) {
        report.memory = value
    ...
    },
    {
      source: 'storage',
      provider: 'systeminformation',
      run: async () => ({ devices: normalizeStorage(await callSystemInformation(si, 'fsSize')) }),
    ...
    },
    {
      ...
      provider: 'systeminformation',
      run: async () => {
        const value = await callSystemInformation(si, 'graphics')
        return {
    ...
    },
    {
      source: 'battery',
      provider: 'systeminformation',
      run: async () => normalizeBattery(await callSystemInformation(si, 'battery')),
    ...
    },
    {
      source: 'performance',
      provider: 'systeminformation',
      run: async () => normalizePerformance(await callSystemInformation(si, 'currentLoad')),
      assign(report, value) {
        report.performance = {
          ...report.performance,
          ...value,
...
async function collectSystemReport(options = {}, dependencies = {}) {
  ...
  const results = await Promise.allSettled(
    collectors.map((collector) => runWithTimeout(collector.source, collector.run, timeoutMs, clock))
  )
