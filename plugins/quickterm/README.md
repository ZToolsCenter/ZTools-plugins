...
> 常用文件夹「在终端中打开 / 在资源管理器中打开」快速启动器

## 更新日志

### v1.2.0 - 2026-09-20

- 新增「在资源管理器中打开」：粘贴 / 拖入文件夹秒开资源管理器，拖入文件则定位选中该文件
- 面板条目 hover 新增「打开文件夹」按钮；主输入框提供终端 / 资源管理器双入口与双候选
...
把常用项目目录收藏成快速启动列表，点击即用指定终端打开该目录（hover 点「打开文件夹」用资源管理器直达），省去每次「打开终端 → cd 一长串路径」或在资源管理器里层层点进去的重复操作。
...
| 右键文件夹 →「在资源管理器中打开」 | 用系统资源管理器打开该目录，并自动退出插件 |
| 右键文件 →「在资源管理器中显示」 | 在资源管理器中定位并选中该文件 |
| 主输入框**粘贴 / 输入任意文本** | 选中「在终端中打开」或「在资源管理器中打开」即可：路径形态直接打开；输入收藏名称（如 `babe`）自动匹配收藏列表打开 |
| 主输入框**输入关键字** | 搜索收藏列表（动态推送，终端 / 资源管理器双候选），选中即打开 |
...
- **删除**：hover 条目后点击「删除」
- **打开**：点击条目，在终端中打开该目录；hover 条目后点「打开文件夹」用系统资源管理器直达
...
- 常用文件夹收藏列表，点击即用终端打开，hover「打开文件夹」用资源管理器直达
- 粘贴 / 拖入文件夹秒开终端，无需经过面板；右键可选「在资源管理器中打开」
