...
import { showError, showData, runCodec } from '../codec'

export default function Sha3_512UI({ enterPayload }: AlgorithmProps) {
  ...
  const run = () => {
    const r = runCodec(() => window.services.crypt.sha3_512.digest(input))
  ...
  return (
    <>
      ...
      <div className="ct-output-zone">
        ...
        <Field label="摘要 Hex" help="128 位十六进制，SHA-3 海绵结构" type="textarea" value={output} onChange={() => {}} readOnly variant="output" copyable />
      </div>
