...
export default function Crc32UI({ enterPayload }: AlgorithmProps) {
  ...
  const run = () => {
    const r = runCodec(() => window.services.crypt.crc32.checksum({ input }))
    ...
    <>
      <div className="ct-input-zone">
        ...
        <div className="ct-grid2">
          <Field label="数据" help="参与 CRC32 计算的 UTF-8 文本" type="textarea" value={input} onChange={setInput} copyable />
        </div>
      </div>
      ...
      <div className="ct-output-zone">
        ...
        <Field label="CRC32 Hex" help="8 位十六进制校验和（小写）" type="textarea" value={output} onChange={() => {}} readOnly variant="output" copyable />
      </div>
