...
export default function Sha512UI({ enterPayload }: AlgorithmProps) {
  ...
  const run = () => {
    const r = runCodec(() => window.services.crypt.sha512.digest(input))
  ...
  return (
    <>
      ...
      <div className="ct-output-zone">
        ...
        <Field label="摘要 Hex" help="128 位十六进制，安全强度高于 SHA-256" type="textarea" value={output} onChange={() => {}} readOnly variant="output" copyable />
      </div>
