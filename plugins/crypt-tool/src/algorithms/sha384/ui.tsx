...
export default function Sha384UI({ enterPayload }: AlgorithmProps) {
  ...
  const run = () => {
    const r = runCodec(() => window.services.crypt.sha384.digest(input))
  ...
  return (
    <>
      ...
      <div className="ct-output-zone">
        ...
        <Field label="摘要 Hex" help="96 位十六进制，不可逆" type="textarea" value={output} onChange={() => {}} readOnly variant="output" copyable />
      </div>
