...
export default function Base32UI({ direction = 'encode' }: AlgorithmProps) {
  ...
  const run = () => {
    const r = runCodec(() =>
      dir === 'encode'
        ? window.services.crypt.base32.encode({ input, padding: padding === 'true' })
        : window.services.crypt.base32.decode({ input })
    )
    setError(showError(r))
    setOutput(showData(r))
  }

  const inputLabel = dir === 'encode' ? '明文' : 'Base32'
  const inputHelp = dir === 'encode'
    ? '待编码的 UTF-8 文本'
    : '待解码的 Base32 字符串'
  const outputLabel = dir === 'encode' ? 'Base32' : '明文'
      ...
      <div className="ct-input-zone">
        ...
        <div className="ct-grid2">
          {dir === 'encode' ? (
            <Field
              label="Padding 填充"
              help="编码时是否在尾部补 = 以保证长度为 8 的倍数"
              type="select"
              value={padding}
              onChange={setPadding}
              options={[
                { value: 'true', label: '保留 =' },
                { value: 'false', label: '去除 =' }
              ]}
            />
          ) : null}
          <Field
