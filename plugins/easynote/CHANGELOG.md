...
- **Ctrl+Z 撤回失效**：所见即所得编辑器从未加载撤销历史插件（Milkdown commonmark 预设不内置 history），Ctrl+Z / Ctrl+Y / Ctrl+Shift+Z 一直无效。现正常支持撤回与重做。
- **代码块出不来**：粘贴代码进入代码块后，编辑器没有任何「跳出代码块」的键位（Enter 被块内换行永远截胡）。现在 Ctrl/Cmd+Enter 可在代码块任意位置跳出、下方新建段落；在块尾空行上按 Enter 也会直接跳出（顺带吃掉该空行）。
- **粘贴代码后下方无法书写**：粘贴到笔记末尾的代码块之下自动补一个空段落，光标可直接落点继续输入。
- **代码块内粘贴丢换行**：在代码块内粘贴多行纯文本时换行丢失的问题，现按原样保留。

## 1.9.0 - 2026-09-20

### 新增

- **多便利贴**：支持同时打开多张独立便利贴窗口（最多 8 张），新窗口按级联错开摆放、不记录历史位置；同一张便签重复打开时聚焦已有窗口而不是再开一份，避免双开编辑冲突。超出上限时提示。
- **边缘标签堆叠**：多张便签最小化为边缘标签时自动纵向错开摆放、不互相覆盖；每张标签独立拖动，点击/关闭都只作用于自己对应的那张便签。
- **便利贴管理**：主页显示当前打开的便利贴数量（含最小化成标签的），并提供「全部关闭」按钮——整体二次确认后一次收掉全部桌面便利贴（未保存的修改会丢失，不做逐窗确认）。

### 变更

- **保存并发安全**：保存便签改为每次落盘前重新读取最新数据合并写入（只替换当前便签那一条），多窗口同时保存不再互相覆盖丢数据。
- **关闭行为**：关闭单张便利贴只关闭该窗口；关闭最后一张即结束插件进程（与单窗口时代一致），退出动作由该便利贴窗口自己执行（沿用验证过的可靠退出路径）；主页「全部关闭」同样以最后一张窗口执行退出收尾，不会出现进程残留。

## 1.8.1 - 2026-09-16

### 修复

- **复制内容还原为原文**：粘贴进便利贴的文本复制出来带行尾 `\`、`\&` 转义或多余空行的问题。复制原文/纯文本时自动还原编辑器序列化痕迹——转义标点、硬换行、段落间隔；粘贴带入的换行与空行、手动 Enter 添加的行，复制出来都与原输入一致（代码围栏内内容不改动）。

## 1.8.0 - 2026-09-11

### 新增

- **便利贴最小化**：便利贴右上角新增「最小化」，收成吸附在屏幕边缘的竖排标签；标签按笔记 / 待办分别配色、可拖动（上下挪 / 换边 / 换屏，松手自动贴回边缘），点击标签即可还原便签，未保存内容不丢。
